...
env = CRNOMAEnv(time_slots=1000)
# env.lambda_arrival = 0.7
# ✅ Environment Details
...
for e in range(episodes):
    ...
    cumulative_throughput = 0  # ✅ For throughput
    ...
    for t in range(env.T):
        ...
        if len(valid_actions) == 0:
            ...
            log_prob = np.log(1.0 / action_size)
        else:
            action, log_prob = agent.get_action(state, valid_actions)
        ...
        # Check if the SU transmitted successfully
    ...
    avg_throughput = cumulative_throughput / env.T  # ✅ Average throughput
...
# ✅ Save Actor Model
agent.actor.save("ppo_cr_noma_actor.h5")
print("Actor model saved as ppo_cr_noma_actor.h5")

# ✅ Save Critic Model
agent.critic.save("ppo_cr_noma_critic.h5")
print("Critic model saved as ppo_cr_noma_critic.h5")


# ✅ Plot Average Sum of AoI vs Episodes
plt.figure(figsize=(8, 6))
plt.plot(range(1, episodes + 1), aoi_avg_history, marker='o', linestyle='-', color='b')
plt.title('Average Sum of AoI vs Episodes')
plt.xlabel('Episodes')
plt.ylabel('Average Sum of AoI')
plt.grid(True)
plt.tight_layout()
plt.savefig('Avg_AoI_vs_Episodes_PPO.png')
# np.save("ppo_aoi_curve.npy", aoi_avg_history)
plt.show()
