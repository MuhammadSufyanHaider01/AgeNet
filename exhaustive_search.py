...
T = 10000 # time slots per episode
episodes = 100
num_SUs = 3
    ...
    for t in range(T):
        ...
        for u in users:
            for _ in range(arrivals[u]):
                Q[u].append(t)  # <-- store generation time
                if len(Q[u]) > Q_max:  # buffer size = 2
                    Q[u].pop(0)
        ...
        for u in users:
            ...
            # Apply action
            AoI_temp[u] = t - Q_temp[u][0]  # <-- accurate AoI
            Q_temp[u].pop(0)
            B_temp[u] -= E_tx
...
# np.save("exhaustive_search_aoi_curve.npy", aoi_avg_history)
plt.show()
