...
class CRNOMAEnv(gym.Env):
    def __init__(self, time_slots=250):
        ...
        self.action_space = spaces.Discrete(self.num_SUs)
    ...
    def model_energy_harvest(self, P_PU, h_gain, E_max=0.04, k=1000):
        P_recv = P_PU * h_gain
    ...
    def step(self, action):
        ...
        selected_su = self.users[action]
        reward = 0

        h_pu = self.compute_large_scale_gain(self.distances['PU']) * self.compute_small_scale_gain()
        h_su_to_pu = {u: self.compute_large_scale_gain(self.distances_SU_to_PU[u]) * self.compute_small_scale_gain() for u in self.users}
        ...
        for u in self.users:
            if u == selected_su and action in valid_actions and self._PU_SINR_ok(u, h_pu, h_su_to_pu[u]):
                self.packet_count[u] += 1
                gen_time = self.Q[u].pop(0)
                self.AoI[u] = self.time_slot - gen_time
                self.B[u] -= self.E_tx
                reward += 1.5  # Base reward
            else:
                if len(self.Q[u]) > 0:
                    self.AoI[u] += 1
        ...
        if self.time_slot > 10:
            ...
            reward += 3 * np.log1p(aoi_reduction)
            reward -= 2 * np.log1p(aoi_increase)
            reward += 0.1 * aoi_steady

            # ⚖️ AoI variance reduction
            aoi_values = np.array([self.AoI[u] for u in self.users])
            aoi_std = np.std(aoi_values)
            reward -= 3* aoi_std

        # 🎯 Encourage selecting max AoI user
        max_aoi_user = max(self.users, key=lambda u: self.AoI[u])
        if selected_su == max_aoi_user and action in valid_actions:
            reward += 2.5

        # 📦 Dropped packets penalty
        reward -= 1.5 * dropped_packets
